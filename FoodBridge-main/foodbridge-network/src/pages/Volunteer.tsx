...
import { Truck, MapPin, Navigation, PackageCheck, CheckCircle2, ExternalLink, List, Map as MapIcon, AlertTriangle, ShieldAlert, HeartHandshake, Sparkles } from "lucide-react";
...
// --- MOCK SPONSORS LIST (Hackathon Demo) ---
const SPONSORS = [
    "Google CSR", "Tech Mahindra Foundation", "Amazon Care", 
    "Zomato Feeding India", "Swiggy Genie", "Tata Trusts", "Local Lions Club"
];

// Helper to get a consistent sponsor for a specific task ID
const getSponsor = (id: string) => {
    if (!id) return SPONSORS[0];
    // Use the last character of the ID to pick a sponsor deterministically
    const index = id.charCodeAt(id.length - 1) % SPONSORS.length;
...
const Volunteer = () => {
  ...
  const handleAccept = async (id: string) => {
    // Determine sponsor for visual feedback
    const sponsorName = getSponsor(id);
    ...
    // Custom Toast for Sponsorship
    toast.success(
        <div className="flex flex-col">
            <span className="font-bold">Trip Accepted!</span>
            <span className="text-xs opacity-90 flex items-center gap-1">
                <Sparkles size={12} className="text-yellow-400" /> Sponsored by {sponsorName}
            </span>
        </div>
    );
            ...
            {activeTab === "routes" && (
                <motion.div key="rt" initial={{opacity:0, y:10}} animate={{opacity:1, y:0}} exit={{opacity:0, y:-10}} className="space-y-4">
                    {!currentTarget ? (
                        ...
                        <>
                            {/* TARGET #1 - WITH SPONSOR BADGE */}
                            <div className={`glass-card p-0 border-l-4 relative overflow-hidden ${currentTarget.urgent ? 'border-red-500 bg-red-500/10' : 'border-emerald-500 bg-emerald-500/5'}`}>
                                
                                {/* SPONSORSHIP BANNER */}
                                <div className="absolute top-0 right-0 bg-gradient-to-l from-yellow-400 to-amber-500 text-black text-[10px] font-bold px-3 py-1 rounded-bl-xl shadow-md flex items-center gap-1 z-10">
                                    <HeartHandshake className="w-3 h-3" /> Sponsored by {getSponsor(currentTarget.task._id)}
                                </div>
